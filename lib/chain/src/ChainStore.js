...
class ChainStore {
                    ...
                    if (optional_object) {
                        ...
                        if (delta < 60) {
                            ...
                            if (reconnectCounter > 5) {
                                this.subError = Error("ChainStore sync error, please check your system clock");
    ...
    onUpdate(updated_objects) /// map from account id to objects
    {
        for (let a = 0; a < updated_objects.length; ++a) {
            for (let i = 0; i < updated_objects[a].length; ++i) {
                ...
                if (ChainValidation.is_object_id(obj)) { /// the object was removed
        ...
        if (!this.dispatched) {
            ...
            this.timeout = setTimeout(() => {
                ...
                this.subscribers.forEach((callback) => {
                    let tmp ;
                    if(obj && obj.get('operation_id')) 
                        tmp = this.getObject(obj.get('operation_id')).toJS();
                    callback(tmp)
                })
     ...
     * @param uid 账户的数字id
     */
    getAccountsByUid(uid) {
        let account = this.accounts_by_uid.get(uid);
    ...
    fetchAccountByUid(account_uid) {
        let par = [];
        if (ChainValidation.is_account_uid(account_uid)) {
            par.push(account_uid);
        }
        else
            throw Error("argument is not an account uid: " + account_uid);

        return new Promise((resolve, reject) => {
            Apis.instance().db_api().exec("get_accounts_by_uid", [par])
                .then(optional_account_object => {
                    if (optional_account_object && optional_account_object.length > 0 && optional_account_object[0] != null) {
                        this.accounts_by_uid = this.accounts_by_uid.set(optional_account_object[0].uid, optional_account_object[0]);
                        this.notifySubscribers();
                        resolve(optional_account_object[0])
                    }
                    else {
                        this.accounts_by_uid = this.accounts_by_uid.set(account_uid, null)
                        this.notifySubscribers()
                        resolve(null)
                    }
                }, reject)
     ...
     */
    getAccountBalances(uid, assets = [0]) {
        if (!Array.isArray(assets))
            throw Error("argument is not an array: " + assets);
        let balances = this.balance_objects_by_uid.get(uid);
    ...
    fetchBalancesByUid(uid, assets = [0]) {
        if (!ChainValidation.is_account_uid(uid)) {
            throw Error("argument is not an account uid: " + uid);
        }
        return new Promise((resolve, reject) => {
            Apis.instance().db_api().exec("get_account_balances", [uid, assets])
                .then(balance_object => {
                    if (balance_object && balance_object.length > 0) {
                        let mArray = this.balance_objects_by_uid.get(uid);
                        if (mArray === undefined) {
                            this.balance_objects_by_uid = this.balance_objects_by_uid.set(uid, balance_object);
                        } else {
                            //合并两个数组的资产数据
                            let have = false;
                            let tmp = [];
                            balance_object.forEach(a => {
                                have = false;
                                mArray.forEach(b => {
                            ...
                            if (tmp.length > 0) {
                                let newArray = mArray.concat(tmp);
                                this.balance_objects_by_uid = this.balance_objects_by_uid.set(uid, newArray);
                            } else {
                                this.balance_objects_by_uid = this.balance_objects_by_uid.set(uid, mArray);
                            }
                        }
                        this.notifySubscribers();
                        ...
                        this.balance_objects_by_uid = this.balance_objects_by_uid.set(uid, null)
                        this.notifySubscribers()
                        resolve(null)
                    }
                }, reject)
     ...
     * @returns {*}
     */
    getPost(platform_pid, poster_uid, post_pid) {
        let fullid = "" + platform_pid + poster_uid + post_pid;
        let post = this.posts_by_fullid.get(fullid);
        if (post === undefined) {
    ...
    fetchPost(platform_pid, poster_uid, post_pid) {
        if (!ChainValidation.is_account_uid(poster_uid)) {
            throw Error("argument is not an account poster_uid: " + poster_uid);
        }
        return new Promise((resolve, reject) => {
            Apis.instance().db_api().exec("get_post", [platform_pid, poster_uid, post_pid])
                .then(post_object => {
                    let fullid = "" + platform_pid + poster_uid + post_pid;
                    if (post_object) {
                        this.posts_by_fullid = this.posts_by_fullid.set(fullid, post_object);
                        this.notifySubscribers();
                        resolve(post_object);
                    }
                    else {
                        this.posts_by_fullid = this.posts_by_fullid.set(fullid, null);
                        this.notifySubscribers();
                        resolve(null);
                    }
                }, reject)
     ...
     * @param poster
     */
    fetchPostsByPlatformPoster(platform_pid, startTime, endTime, limit = 20, poster = null) {
        if (!(startTime instanceof Date))
            throw Error("argument is not date type: " + startTime);
        if (!(endTime instanceof Date))
            throw Error("argument is not date type: " + endTime);
        if (limit < 0) limit = 20;
        ...
        let s = startTime.toISOString().split('.')[0];
        let e = endTime.toISOString().split('.')[0];

        return new Promise((resolve, reject) => {
            Apis.instance().db_api().exec("get_posts_by_platform_poster", [platform_pid, poster, [s, e], limit])
                .then(post_objects => {
                    if (post_objects) {
                        let fullid = "";
                        post_objects.forEach(p => {
                            fullid = "" + p.platform + p.poster + p.post_pid;
                            this.posts_by_fullid = this.posts_by_fullid.set(fullid, p);
                        });
                        this.notifySubscribers();
     ...
     */
    fetchPostsByParent(platform_pid, startTime, endTime, limit = 20, parent = null) {
        if (!(startTime instanceof Date))
            throw Error("argument is not date type: " + startTime);
        if (!(endTime instanceof Date))
            throw Error("argument is not date type: " + endTime);
        ...
        return new Promise((resolve, reject) => {
            Apis.instance().db_api().exec("get_posts_by_parent", [platform_pid, parent, [s, e], limit])
                .then(post_objects => {
                    if (post_objects) {
                        let fullid = "";
                        post_objects.forEach(p => {
     ...
     * @param limit
     */
    getRelativeAccountHistory(account_uid, limit = 20) {
        let account_history = this.account_historys.get(account_uid);
        if (account_history === undefined) {
            this.fetchRelativeAccountHistory(account_uid, null, 0, limit);
        } else {
            let tmpAH = account_history[account_history.length - 1];
            if (tmpAH.length == 2) {
                if (tmpAH[0] == 1) {
                    this.fetchRelativeAccountHistory(account_uid, null, 0, limit);
                } else {
     ...
     * @param start
     */
    fetchRelativeAccountHistory(account_uid, op_type = null, stop = 0, limit = 20, start = 0) {
        if (!ChainValidation.is_account_uid(account_uid)) {
            throw Error("argument is not uid: " + account_uid);
        }
        return new Promise((resolve, reject) => {
            Apis.instance().history_api().exec("get_relative_account_history", [account_uid, op_type, stop, limit, start])
                .then(history_objects => {
                    if (history_objects) {
                        if (op_type == null) {
                            if (start == 0) {
                                this.account_historys = this.account_historys.set(account_uid, history_objects);
                            } else {
                                let current_history = this.account_historys.get(account_uid);
                                if (!current_history) current_history = Immutable.List()
                                let updated_history = Immutable.fromJS(current_history);
                                updated_history = updated_history.withMutations(list => {
                                    for (let i = 0; i < history_objects.length; ++i) {
                                        let tmp = current_history.find(ch => {
                                ...
                                this.account_historys = this.account_historys.set(account_uid, updated_history);
                            }
                            this.notifySubscribers();
                        }
     ...
     */
    fetchAccountStatisticsByUid(uid){
        return new Promise((resolve, reject) => {
            if (!ChainValidation.is_account_uid(uid)) {
     ...
     */
    fetchPlatformByUid(uid){
        return new Promise((resolve, reject) => {
            if (!ChainValidation.is_account_uid(uid)){
                reject({code: 2002, message: 'invalid account uid'});
            }else{
                Apis.instance().db_api().exec("get_platform_by_account", [uid]).then(platform => {
                    if(platform){
                        resolve(platform);   
                    }else{
     ...
     */
    fetchPlatforms(pid, limit = 10, sortType = 1){
        return new Promise((resolve, reject) => {
            Apis.instance().db_api().exec('lookup_platforms', [pid, limit, sortType]).then(res => {
                resolve(res);
            }).catch(e => {
                reject(e);
            });
     ...
     * 查询资产列表
     */
    fetchAssets(lower_bound_symbol = null){
        let size = 100;
        return new Promise((resolve, reject) => {
            Apis.instance().db_api().exec("list_assets", [lower_bound_symbol, size]).then(res => {
                if(lower_bound_symbol) res.shift();
                let assetIds = [];
                for(let o of res){
                    assetIds.push(o.dynamic_asset_data_id);
                }
                Apis.instance().db_api().exec('get_objects', [assetIds]).then(dyms => {
                    for(let i = 0 ; i < dyms.length ; i ++){
                        let dym = dyms[i];
                    ...
                    if(res.length >= size - 1) {
                        this.fetchAssets(res[res.length - 1].symbol).then(_res => {
     ...
     */
    fetchAsset(query){
        let apiType = 'lookup_asset_symbols';
        if(typeof query == 'number') apiType = 'get_assets'; 
        return new Promise((resolve, reject) => {
           Apis.instance().db_api().exec(apiType, [[query]]).then(res => {
               if(res.length > 0){
                   let asset = res[0];
                   if(asset){
                        Apis.instance().db_api().exec('get_objects', [[asset.dynamic_asset_data_id]]).then(dynamic => {
                                let dym = dynamic[0];
    ...
    fetchAccountBalances(uid, assets = []){
        return new Promise((resolve, reject) => {
            ...
            let promise_balances = Apis.instance().db_api().exec('get_account_balances', [uid, assets]);
            let promise_core_balance = Apis.instance().db_api().exec("get_full_accounts_by_uid", [[uid], {fetch_statistics: true}]);

            Promise.all([promise_core_balance, promise_balances]).then(res => {
                if(res[0].length == 0) reject(new Error('Unknown account uid'));
                let _statistics = res[0][0][1].statistics;
                let _assets = [];
                let _assets_id = [];
                for(let a of res[1]){
                    if(a.asset_id == 0) 
                        a.amount = Long.fromValue(a.amount).add(_statistics.prepaid).toString();
                    _assets.push(a);
                    _assets_id.push(a.asset_id);
                }
                Apis.instance().db_api().exec('get_assets', [_assets_id]).then(a_res => {
