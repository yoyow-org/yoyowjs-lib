...
const HEX_DUMP = process.env.npm_config_serializerdump;
...
Types.jsenum = function (type_map)
{
    ...
    return {
        ...
        fromObject(object){
            v.required(object);
            return v.to_long(type_map[object]);
        },
        toObject(object, debug = {}){
            if (debug.use_default && object === undefined) {
                return "0";
            ...
            for(var i in type_map){
                if(v.to_long(type_map[i]).eq(v.to_long(object)));
                result = i;
            }
            return result;
        }
...
Types.name = {
    ...
    longToString(l){
        //max length of a 'name' type
        const max_name_length = 13;
        const char_map = ".12345abcdefghijklmnopqrstuvwxyz";
        const chars = new Array(max_name_length);
        chars.fill(char_map[0]);
        let tmp = Long.fromValue(l);
        for(let i=0;i<max_name_length;++i){
            let c = char_map[tmp.and((i==0 ? 0x0f : 0x1f))];
...
let longCmp = (a, b) => a.gt(b) ? 1 : a.lt(b) ? -1 : 0
...
let sortOperation = (array, st_operation) =>
    st_operation.nosort ? array :
        st_operation.compare ? array.sort((a, b) => st_operation.compare(firstEl(a), firstEl(b))) : // custom compare operation
            array.sort((a, b) =>
                v.is_long(firstEl(a)) && v.is_long(firstEl(b)) ? longCmp(firstEl(a),firstEl(b)) :
