...
function deterministicGenerateK(curve, hash_buff, d, checkSig, nonce) {

  (0, _enforce_types2.default)('Buffer', hash_buff);
  (0, _enforce_types2.default)(_bigi2.default, d);

  if (nonce) {
    hash_buff = _hash2.default.sha256(Buffer.concat([hash_buff, new Buffer(nonce)]));
  }

  // sanity check
  _assert2.default.equal(hash_buff.length, 32, 'Hash must be 256 bit');
  ...
  // Step D
  k = _hash2.default.HmacSHA256(Buffer.concat([v, new Buffer([0]), x, hash_buff]), k);

  // Step E
  v = _hash2.default.HmacSHA256(v, k);

  // Step F
  k = _hash2.default.HmacSHA256(Buffer.concat([v, new Buffer([1]), x, hash_buff]), k);
  ...
  while (T.signum() <= 0 || T.compareTo(curve.n) >= 0 || !checkSig(T)) {
    k = _hash2.default.HmacSHA256(Buffer.concat([v, new Buffer([0])]), k);
    v = _hash2.default.HmacSHA256(v, k);
...
function sign(curve, hash_buff, d, nonce) {

  var e = _bigi2.default.fromBuffer(hash_buff);
  var n = curve.n;
  var G = curve.G;

  var r, s;
  var k = deterministicGenerateK(curve, hash_buff, d, function (k) {
...
function verify(curve, hash_buff, signature, Q) {
