...
var TransactionBuilder = function () {
    ...
    TransactionBuilder.prototype.finalize = function finalize() {
        ...
        return new Promise(function (resolve, reject) {
            ...
            resolve(_yoyowjsWs.Apis.instance().db_api().exec("get_objects", [["2.1.0"]]).then(function (r) {
                head_block_time_string = r[0].time;
                if (_this.expiration === 0) _this.expiration = base_expiration_sec() + _yoyowjsWs.ChainConfig.expire_in_secs;
                ...
                _this.tr_buffer = _serializer.ops.transaction.toBuffer(_this);
            }));
        });
    };
    ...
    TransactionBuilder.prototype.get_type_operation = function get_type_operation(name, operation) {
        ...
        if (name === 'proposal_create') {
            ...
            operation.expiration_time || (operation.expiration_time = base_expiration_sec() + _yoyowjsWs.ChainConfig.expire_in_secs_proposal);
            if (requiresReview) {
                operation.review_period_seconds = extraReview + Math.max(committee_min_review, 24 * 60 * 60 || _yoyowjsWs.ChainConfig.review_in_secs_committee);
                /*
    ...
    TransactionBuilder.prototype.update_head_block = function update_head_block() {
        return Promise.all([_yoyowjsWs.Apis.instance().db_api().exec("get_objects", [["2.0.0"]]), _yoyowjsWs.Apis.instance().db_api().exec("get_objects", [["2.1.0"]])]).then(function (res) {
            var g = res[0],
                r = res[1];
    ...
    TransactionBuilder.prototype.get_fees_by_ops = function get_fees_by_ops() {
        ...
        var promises = [_yoyowjsWs.Apis.instance().db_api().exec("get_required_fee_data", [operations])];
        if (pay_uid != null) {
            promises.push(_yoyowjsWs.Apis.instance().db_api().exec("get_full_accounts_by_uid", [[pay_uid], { fetch_statistics: true }]));
        }
        return Promise.all(promises).then(function (results) {
            var statistics = null;
            if (pay_uid != null) statistics = results[1][0][1].statistics;
            var fees = results[0];
            var min_fees = _bytebuffer.Long.ZERO,
                min_real_fees = _bytebuffer.Long.ZERO;
            if (fees.length > 0) {
                var _i = 0;
                if (pay_uid != null) {
                    for (_i = 0; _i < fees.length; _i++) {
                        ...
                        if (fees[_i].fee_payer_uid == pay_uid) {
                            //支付人是指定账户时才统计
                            min_fees = min_fees.add(_bytebuffer.Long.fromValue(fees[_i].min_fee));
                            min_real_fees = min_real_fees.add(_bytebuffer.Long.fromValue(fees[_i].min_real_fee));
                        }
                    }
                } else {
                    for (_i = 0; _i < fees.length; _i++) {
    ...
    TransactionBuilder.prototype.set_required_fees = function set_required_fees(pay_uid) {
        ...
        var asset_id = 0;
        var promises = [_yoyowjsWs.Apis.instance().db_api().exec("get_required_fee_data", [operations]), _yoyowjsWs.Apis.instance().db_api().exec("get_full_accounts_by_uid", [[pay_uid], { fetch_statistics: true }])];

        return Promise.all(promises).then(function (results) {
            //console.log("statistics:", results[1][0][1].statistics);
            var statistics = results[1][0][1].statistics;
            var fees = results[0];
            var fee_total = _bytebuffer.Long.ZERO; // Long.fromValue(fees[0][0]);
            for (var _i2 = 0, fee; _i2 < fees.length; _i2++) {
                //计算总手续费
                if (fees[_i2].fee_payer_uid === pay_uid) {
                    //支付人是指定账户时才统计
                    fee = _bytebuffer.Long.fromValue(fees[_i2].min_fee);
                    fee_total = fee_total.add(fee);
                }
            }

            var core_balance = _bytebuffer.Long.fromValue(statistics.core_balance);
            var prepaid_balance = _bytebuffer.Long.fromValue(statistics.prepaid);
            var csaf_balance = _bytebuffer.Long.fromValue(statistics.csaf);
            ...
            var set_fee = function set_fee(operation) {
                if (!operation.fee || operation.fee.total.amount === 0 || operation.fee.total.amount.toString && operation.fee.total.amount.toString() === "0") {
                    var fee_mast = _bytebuffer.Long.fromValue(fees[fee_index].min_real_fee); //最低必须用币支付的费用
                    var tmpFee = { total: { amount: _bytebuffer.Long.fromValue(fees[fee_index].min_fee), asset_id: 0 } }; //最低总费用
                    ...
                    if (csaf.gt(0)) {
                        ...
                        if (csaf_balance.gte(csaf) && useCsaf) {
                            ...
                            //再用零钱/余额
                            var old = _bytebuffer.Long.ZERO;
                            if (isBalance) {
                                old = options.from_balance ? options.from_balance.amount : _bytebuffer.Long.ZERO;
                                options.from_balance = { amount: csaf.sub(fc).add(old), asset_id: 0 };
                            } else {
                                old = options.from_prepaid ? options.from_prepaid.amount : _bytebuffer.Long.ZERO;
    ...
    TransactionBuilder.prototype.get_potential_signatures = function get_potential_signatures() {
        ...
        return Promise.all([_yoyowjsWs.Apis.instance().db_api().exec("get_potential_signatures", [tr_object]), _yoyowjsWs.Apis.instance().db_api().exec("get_potential_address_signatures", [tr_object])]).then(function (results) {
    ...
    TransactionBuilder.prototype.get_required_signatures = function get_required_signatures(available_keys) {
        ...
        var tr_object = _serializer.ops.signed_transaction.toObject(this);
        return _yoyowjsWs.Apis.instance().db_api().exec("get_required_signatures", [tr_object, available_keys]).then(function (required_public_keys) {
    ...
    TransactionBuilder.prototype.get_transaction_hex = function get_transaction_hex() {
        var tr_object = _serializer.ops.signed_transaction.toObject(this);
        return _yoyowjsWs.Apis.instance().db_api().exec("get_transaction_hex", [tr_object]).then(function (hex) {
            return hex;
        });
    };
    ...
    TransactionBuilder.prototype.sign = function sign() {
        var chain_id = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : _yoyowjsWs.Apis.instance().chain_id;
    ...
    TransactionBuilder.prototype.serialize = function serialize() {
        return _serializer.ops.signed_transaction.toHex(this);
    };
...
function _broadcast(was_broadcast_callback) {
    ...
    return new Promise(function (resolve, reject) {
        ...
        var tr_object = _serializer.ops.signed_transaction.toObject(_this4);
        var tr_res = {
            txid: _this4.id(),
            block_num: _this4.head_block_number()
        };
        _yoyowjsWs.Apis.instance().network_api().exec("broadcast_transaction_with_callback", [function (res) {
            ...
            eobj.digest = _ecc.hash.sha256(_this4.tr_buffer).toString('hex');
