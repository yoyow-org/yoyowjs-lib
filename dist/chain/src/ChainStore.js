...
var ChainStore = function () {
    ...
    ChainStore.prototype.clearCache = function clearCache() {
        ...
        this.accounts_by_uid = _immutable2.default.Map();
        this.balance_objects_by_uid = _immutable2.default.Map();
        this.posts_by_fullid = _immutable2.default.Map();
        this.account_historys = _immutable2.default.Map();
    ...
    ChainStore.prototype.init = function init() {
        ...
        var _init = function _init(resolve, reject) {
            var db_api = _yoyowjsWs.Apis.instance().db_api();
            if (!db_api) {
                ...
                for (var i = 0; i < optional_objects.length; i++) {
                    ...
                    if (optional_object) {
                        ...
                        if (delta < 60) {
                            ...
                            _yoyowjsWs.Apis.instance().db_api().exec("set_subscribe_callback", [_this.onUpdate.bind(_this), true]).then(function () {
    ...
    ChainStore.prototype.getAsset = function getAsset(id_or_symbol) {
        ...
        _yoyowjsWs.Apis.instance().db_api().exec("lookup_asset_symbols", [[id_or_symbol]]).then(function (asset_objects) {
    ...
    ChainStore.prototype.getAccountRefsOfKey = function getAccountRefsOfKey(key) {
        ...
        if (this.get_account_refs_of_keys_calls.has(key)) return this.account_ids_by_key.get(key);else {
            ...
            _yoyowjsWs.Apis.instance().db_api().exec("get_key_references", [[key]]).then(function (vec_account_id) {
    ...
    ChainStore.prototype.getBalanceObjects = function getBalanceObjects(address) {
        ...
        if (current === undefined) {
            ...
            _yoyowjsWs.Apis.instance().db_api().exec("get_balance_objects", [[address]]).then(function (balance_objects) {
                var set = new Set();
    ...
    ChainStore.prototype.fetchObject = function fetchObject(id) {
        ...
        if (result === undefined) {
            ...
            _yoyowjsWs.Apis.instance().db_api().exec("get_objects", [[id]]).then(function (optional_objects) {
                //if(__LIB_DEBUG__) console.log('... optional_objects',optional_objects ? optional_objects[0].id : null)
    ...
    ChainStore.prototype.fetchAccountByUid = function fetchAccountByUid(account_uid) {
        ...
        var par = [];
        if (_ChainValidation2.default.is_account_uid(account_uid)) {
            par.push(account_uid);
        } else throw Error("argument is not an account uid: " + account_uid);

        return new Promise(function (resolve, reject) {
            _yoyowjsWs.Apis.instance().db_api().exec("get_accounts_by_uid", [par]).then(function (optional_account_object) {
    ...
    ChainStore.prototype.fetchBalancesByUid = function fetchBalancesByUid(uid) {
        ...
        if (!_ChainValidation2.default.is_account_uid(uid)) {
            throw Error("argument is not an account uid: " + uid);
        }
        return new Promise(function (resolve, reject) {
            _yoyowjsWs.Apis.instance().db_api().exec("get_account_balances", [uid, assets]).then(function (balance_object) {
    ...
    ChainStore.prototype.fetchPost = function fetchPost(platform_pid, poster_uid, post_pid) {
        ...
        if (!_ChainValidation2.default.is_account_uid(poster_uid)) {
            throw Error("argument is not an account poster_uid: " + poster_uid);
        }
        return new Promise(function (resolve, reject) {
            _yoyowjsWs.Apis.instance().db_api().exec("get_post", [platform_pid, poster_uid, post_pid]).then(function (post_object) {
                var fullid = "" + platform_pid + poster_uid + post_pid;
                if (post_object) {
    ...
    ChainStore.prototype.fetchPostsByPlatformPoster = function fetchPostsByPlatformPoster(platform_pid, startTime, endTime) {
        ...
        return new Promise(function (resolve, reject) {
            _yoyowjsWs.Apis.instance().db_api().exec("get_posts_by_platform_poster", [platform_pid, poster, [s, e], limit]).then(function (post_objects) {
                if (post_objects) {
                    var fullid = "";
    ...
    ChainStore.prototype.fetchPostsByParent = function fetchPostsByParent(platform_pid, startTime, endTime) {
        ...
        if (parent != null) {
            ...
            } else if (!_ChainValidation2.default.is_account_uid(parent[0])) {
        ...
        return new Promise(function (resolve, reject) {
            _yoyowjsWs.Apis.instance().db_api().exec("get_posts_by_parent", [platform_pid, parent, [s, e], limit]).then(function (post_objects) {
                if (post_objects) {
                    var fullid = "";
                    post_objects.forEach(function (p) {
                        fullid = "" + p.platform + p.poster + p.post_pid;
                ...
                if (history_objects) {
                    if (op_type == null) {
                        if (start == 0) {
                            ...
                            var updated_history = _immutable2.default.fromJS(current_history);
        ...
        if (_ChainValidation2.default.is_account_name(id_or_account, true) || id_or_account.substring(0, 4) == "1.2.") {
            ...
            if (!_account2) {
                this.lookupAccountByName(id_or_account).then(function (account) {
                    ...
                    if (_ChainValidation2.default.is_object_id(witness_id)) return _this13.getObject(witness_id, on_update);
        ...
        if (_ChainValidation2.default.is_account_name(id_or_account, true) || id_or_account.substring(0, 4) == "1.2.") {
            ...
            if (!account) {
                this.lookupAccountByName(id_or_account).then(function (account) {
                    ...
                    if (_ChainValidation2.default.is_object_id(committee_id)) return _this14.getObject(committee_id, on_update);
    ...
    ChainStore.prototype.fetchWitnessByAccount = function fetchWitnessByAccount(account_id) {
        ...
        return new Promise(function (resolve, reject) {
            _yoyowjsWs.Apis.instance().db_api().exec("get_witness_by_account", [account_id]).then(function (optional_witness_object) {
    ...
    ChainStore.prototype.fetchCommitteeMemberByAccount = function fetchCommitteeMemberByAccount(account_id) {
        ...
        return new Promise(function (resolve, reject) {
            _yoyowjsWs.Apis.instance().db_api().exec("get_committee_member_by_account", [account_id]).then(function (optional_committee_object) {
    ...
    ChainStore.prototype.fetchFullAccount = function fetchFullAccount(name_or_id) {
        ...
        if (!this.fetching_get_full_accounts.has(name_or_id) || Date.now() - this.fetching_get_full_accounts.get(name_or_id) > 5000) {
            ...
            _yoyowjsWs.Apis.instance().db_api().exec("get_full_accounts", [[name_or_id], true]).then(function (results) {
                if (results.length === 0) {
                    if (_ChainValidation2.default.is_object_id(name_or_id)) {
                ...
                console.log("Error: ", error);
                if (_ChainValidation2.default.is_object_id(name_or_id)) _this17.objects_by_id = _this17.objects_by_id.delete(name_or_id);else _this17.accounts_by_name = _this17.accounts_by_name.delete(name_or_id);
            });
        }
        return undefined;
    };
    ...
    ChainStore.prototype.fetchRecentHistory = function fetchRecentHistory(account) {
        ...
        pending_request.promise = new Promise(function (resolve, reject) {
            _yoyowjsWs.Apis.instance().history_api().exec("get_account_history", [account_id, most_recent, limit, start]).then(function (operations) {
    ...
    ChainStore.prototype.getObjectsByVoteIds = function getObjectsByVoteIds(vote_ids) {
        ...
        if (missing.length) {
            ...
            _yoyowjsWs.Apis.instance().db_api().exec("lookup_vote_ids", [missing]).then(function (vote_obj_array) {
    ...
    ChainStore.prototype.fetchAccountStatisticsByUid = function fetchAccountStatisticsByUid(uid) {
        return new Promise(function (resolve, reject) {
            if (!_ChainValidation2.default.is_account_uid(uid)) {
                ...
                _yoyowjsWs.Apis.instance().db_api().exec("get_full_accounts_by_uid", [[uid], { fetch_statistics: true }]).then(function (res) {
                    if (res.length == 0) {
    ...
    ChainStore.prototype.fetchPlatformByUid = function fetchPlatformByUid(uid) {
        return new Promise(function (resolve, reject) {
            if (!_ChainValidation2.default.is_account_uid(uid)) {
                ...
                _yoyowjsWs.Apis.instance().db_api().exec("get_platform_by_account", [uid]).then(function (platform) {
                    if (platform) {
                        resolve(platform);
                    } else {
    ...
    ChainStore.prototype.fetchPlatforms = function fetchPlatforms(pid) {
        ...
        return new Promise(function (resolve, reject) {
            _yoyowjsWs.Apis.instance().db_api().exec('lookup_platforms', [pid, limit, sortType]).then(function (res) {
                resolve(res);
            }).catch(function (e) {
                reject(e);
            });
    ...
    ChainStore.prototype.fetchAssets = function fetchAssets() {
        ...
        var size = 100;
        return new Promise(function (resolve, reject) {
            _yoyowjsWs.Apis.instance().db_api().exec("list_assets", [lower_bound_symbol, size]).then(function (res) {
                ...
                _yoyowjsWs.Apis.instance().db_api().exec('get_objects', [assetIds]).then(function (dyms) {
    ...
    ChainStore.prototype.fetchAsset = function fetchAsset(query) {
        ...
        return new Promise(function (resolve, reject) {
            _yoyowjsWs.Apis.instance().db_api().exec(apiType, [[query]]).then(function (res) {
                if (res.length > 0) {
                    var asset = res[0];
                    if (asset) {
                        _yoyowjsWs.Apis.instance().db_api().exec('get_objects', [[asset.dynamic_asset_data_id]]).then(function (dynamic) {
                            var dym = dynamic[0];
    ...
    ChainStore.prototype.fetchAccountBalances = function fetchAccountBalances(uid) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            var promise_balances = _yoyowjsWs.Apis.instance().db_api().exec('get_account_balances', [uid, assets]);
            var promise_core_balance = _yoyowjsWs.Apis.instance().db_api().exec("get_full_accounts_by_uid", [[uid], { fetch_statistics: true }]);
            ...
            Promise.all([promise_core_balance, promise_balances]).then(function (res) {
                ...
                for (var _iterator2 = res[1], _isArray2 = Array.isArray(_iterator2), _i4 = 0, _iterator2 = _isArray2 ? _iterator2 : _iterator2[Symbol.iterator]();;) {
                    ...
                    if (a.asset_id == 0) a.amount = _bytebuffer.Long.fromValue(a.amount).add(_statistics.prepaid).toString();
                    _assets.push(a);
                    _assets_id.push(a.asset_id);
                }
                _yoyowjsWs.Apis.instance().db_api().exec('get_assets', [_assets_id]).then(function (a_res) {
